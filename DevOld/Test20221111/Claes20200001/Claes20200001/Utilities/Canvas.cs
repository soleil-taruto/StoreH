...
namespace Charlotte.Utilities
{
	public class Canvas
	{
		...
		public Canvas GetClone()
		{
			...
			dest.DrawImage(this, 0, 0, false);
			return dest;
		}

		public Canvas Blur(int blurLv)
		{
			if (blurLv < 0)
				throw new ArgumentException("Bad blurLv");

			if (blurLv == 0)
				return this.GetClone();

			Canvas canvas_01 = new Canvas(this.W, this.H);

			BlurOnce(this, canvas_01);

			if (blurLv == 1)
				return canvas_01;

			Canvas canvas_02 = new Canvas(this.W, this.H);

			for (int blur = 1; blur < blurLv; blur++)
			{
				BlurOnce(canvas_01, canvas_02);
				SCommon.Swap(ref canvas_01, ref canvas_02);
			}
			return canvas_01;
		}

		private static void BlurOnce(Canvas src, Canvas dest)
		{
			int w = src.W;
			int h = src.H;
				...
				for (int y = 0; y < h; y++)
				{
					...
					for (int xc = -1; xc <= 1; xc++)
					{
						for (int yc = -1; yc <= 1; yc++)
						{
							...
							{
								double sr = src[sx, sy].R / 255.0;
								double sg = src[sx, sy].G / 255.0;
								double sb = src[sx, sy].B / 255.0;
								double sa = src[sx, sy].A / 255.0;
					...
					dest[x, y] = new D4Color(r, g, b, a).ToI4Color();
				}
			}
		}

		public void Deepen(double depth)
		{
			for (int x = 0; x < this.W; x++)
			{
				for (int y = 0; y < this.H; y++)
				{
					double a = this[x, y].A / 255.0;
